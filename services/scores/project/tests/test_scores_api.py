...
class TestScoresService(BaseTestCase):
    ...
    def test_all_scores(self):
        ...
        add_score(1, 2, True)
        add_score(998_877, 878_778, False)
        with self.client:
            ...
            self.assertEqual(998_877, data["data"]["scores"][1]["user_id"])
            self.assertEqual(878_778, data["data"]["scores"][1]["exercise_id"])
    ...
    def test_all_scores_by_user_id(self):
        ...
        add_score(998_877, 878_778, True)
        with self.client:
            ...
            self.assertEqual(998_877, data["data"]["scores"][0]["user_id"])
            self.assertEqual(878_778, data["data"]["scores"][0]["exercise_id"])
            self.assertTrue(data["data"]["scores"][0]["correct"])
            self.assertIn("success", data["status"])
    ...
    def test_single_score_by_user_id(self):
        """Ensure get all scores by user id behaves correctly."""
        score = add_score(998_877, 65479, True)
        with self.client:
            response = self.client.get(
            ...
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
            self.assertEqual(998_877, data["data"]["user_id"])
